...
module yolo_engine #(
    ...
    // AXI Master Ports (Read & Write - 생략 없이 기존과 100% 동일 연결)
...
always @ (posedge clk or negedge rstn) begin
    if(~rstn) begin ap_start <= 0; dram_base_addr_rd <= 0; dram_base_addr_wr <= 0; end 
    else begin 
        if(!ap_start && i_ctrl_reg0[0]) begin ap_start <= 1; dram_base_addr_rd <= i_ctrl_reg1; dram_base_addr_wr <= i_ctrl_reg2; end 
        else if (ap_start && network_done) begin ap_start <= 0; end 
    end 
end

// 1. DMA READ 
axi_dma_ctrl #(.BIT_TRANS(16)) u_dma_ctrl(
    .clk(clk), .rstn(rstn), .i_start(i_ctrl_reg0[0]), .i_base_address_rd(dram_base_addr_rd), .i_base_address_wr(dram_base_addr_wr),
...
always @(posedge clk or negedge rstn) begin
    if (!rstn) begin
        ...
        if (i_ctrl_reg0[0]) begin total_read_cnt <= 0; addra_64 <= 0; pack_state <= 0; end
...
always @(posedge clk or negedge rstn) begin
    if(!rstn) begin out_ram_addra <= 0; out_ram_wea <= 0; write_p1_pending <= 0; out_ram_dia <= 0; end 
    else begin
        if (out_valid) begin
        ...
        end else begin out_ram_wea <= 0; end
        if (out_ram_wea) out_ram_addra <= out_ram_addra + 1; 
        if (i_ctrl_reg0[0]) out_ram_addra <= 0; // 프레임 시작 시 주소 초기화
    end
end

reg [15:0] wr_blk_idx; reg wr_start_dma; wire write_done; wire indata_req_wr; wire [31:0] write_data;
always @(posedge clk or negedge rstn) begin
    if (!rstn) begin wr_blk_idx <= 0; wr_start_dma <= 0; network_done <= 0; end 
    else if (layer_done) begin 
        if (wr_blk_idx < max_req_blk_idx) begin
            if (!wr_start_dma && !write_done) wr_start_dma <= 1;
            else if (write_done) begin wr_start_dma <= 0; wr_blk_idx <= wr_blk_idx + 1; end
        end else network_done <= 1;
    end else if (i_ctrl_reg0[0]) begin network_done <= 0; wr_blk_idx <= 0; end
end

wire [31:0] safe_write_addr = dram_base_addr_wr + (wr_blk_idx * 16 * 4);
reg [15:0] my_write_data_cnt;
always @(posedge clk or negedge rstn) begin
...
dpram_wrapper #(.DEPTH(65536), .AW(16), .DW(32)) u_out_ram (
    ...
    .enb(1'b1), .addrb(my_write_data_cnt), .dob(write_data) 
);
...
axi_dma_wr #(.BITS_TRANS(16), .OUT_BITS_TRANS(16), .AXI_WIDTH_USER(1), .AXI_WIDTH_ID(4), .AXI_WIDTH_AD(AXI_WIDTH_AD), .AXI_WIDTH_DA(AXI_WIDTH_DA), .AXI_WIDTH_DS(AXI_WIDTH_DS)) u_dma_write(
    ...
    .M_WID(M_WID), .M_WDATA(M_WDATA), .M_WSTRB(M_WSTRB), .M_WLAST(M_WLAST), .M_WVALID(M_WVALID), .M_WREADY(M_WREADY),
